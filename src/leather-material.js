...
import { LEATHER_CONFIG, RUBBER_CONFIG, TOP_CAP_CONFIG } from "./leather-config.js";

// Re-export config để main.js có thể import
export { LEATHER_CONFIG, RUBBER_CONFIG, TOP_CAP_CONFIG };
...
export function loadLeatherNormal() {
  if (leatherNormalPromise) return leatherNormalPromise;
...
export function createNormalMap(width, height) {
  ...
  // Mặc định: flat normal (neutral)
  ctx.fillStyle = `rgb(128, 128, 255)`;
  ctx.fillRect(0, 0, width, height);
  
  // Vùng da: dùng leather normal map FILE nếu đã load
  if (leatherNormalImage) {
    const leatherY = Math.floor(LEATHER_START_RATIO * height);
    const leatherEndY = Math.floor(LEATHER_END_RATIO * height);
    const leatherHeight = leatherEndY - leatherY;
    
    // Scale texture từ config
    const tileWidth = leatherNormalImage.width * LEATHER_CONFIG.textureScaleX;
    const tileHeight = leatherNormalImage.height * LEATHER_CONFIG.textureScaleY;
    
    // Tính số lần repeat
    const repeatX = Math.ceil(width / tileWidth);
    const repeatY = Math.ceil(leatherHeight / tileHeight);
    
    // Clip vào vùng da
    ctx.save();
    ctx.beginPath();
    ctx.rect(0, leatherY, width, leatherHeight);
    ctx.clip();
    
    // Vẽ tile leather normal từ FILE
    for (let rx = 0; rx < repeatX; rx++) {
      for (let ry = 0; ry < repeatY; ry++) {
        ctx.drawImage(
          leatherNormalImage,
          rx * tileWidth,
          leatherY + ry * tileHeight,
          tileWidth,
          tileHeight
        );
      }
    ...
    console.log("[NormalMap] Applied leather normal FILE to region:", leatherY, "-", leatherEndY);
  } else {
...
export function createRubberNormalTexture(width = 512, height = 512) {
  ...
  for (let i = 0; i < tinyCount; i++) {
    ...
    const alpha = 0.14 + Math.random() * 0.10; // hạt nhỏ mịn hơn, ít sáng
  ...
  if (bumperLogoImage && RUBBER_CONFIG.logo.enabled && RUBBER_CONFIG.logo.emboss) {
    ...
    if (RUBBER_CONFIG.logo.flipX || RUBBER_CONFIG.logo.flipY) {
      ...
      tempCtx.scale(
        RUBBER_CONFIG.logo.flipX ? -1 : 1,
        RUBBER_CONFIG.logo.flipY ? -1 : 1
      );
...
export function createTopCapTextureWithLogo(originalTexture, width = 512, height = 512) {
  ...
  if (topCapLogoImage && TOP_CAP_CONFIG.logo.enabled) {
    ...
    const radius = TOP_CAP_CONFIG.logo.topSurfaceOnly && TOP_CAP_CONFIG.logo.topSurfaceUV
      ? (TOP_CAP_CONFIG.logo.topSurfaceUV.radius || 0.2) * Math.min(width, height)
      : null;
  ...
  ctx.globalCompositeOperation = 'screen';
    ...
    if (TOP_CAP_CONFIG.logo.flipX || TOP_CAP_CONFIG.logo.flipY) {
      ctx.translate(logoX + logoW / 2, logoY + logoH / 2);
      ctx.scale(
        TOP_CAP_CONFIG.logo.flipX ? -1 : 1,
        TOP_CAP_CONFIG.logo.flipY ? -1 : 1
      );
      ctx.drawImage(colorCanvas, -logoW / 2, -logoH / 2, logoW, logoH);
    } else {
